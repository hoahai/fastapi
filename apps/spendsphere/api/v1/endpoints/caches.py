...
def _normalize_cache_requests(values: list[str] | None) -> list[str]:
    if not values:
        return ["account_codes", "google_ads_clients", "google_sheets"]
...
@router.post(
    ...
    description=(
        "Refreshes account code, Google Ads clients, and Google Sheets caches "
...
def refresh_cache_route(
    caches: list[str] | None = Query(
        ...
        description=(
            ...
            "google_ads_clients, google_sheets. Can be repeated or comma-separated."
        ),
    ),
):
    """
    ...
    if "google_ads_clients" in requested:
        clients = get_ggad_accounts(refresh_cache=True)
        response["googleAdsClients"] = len(clients)

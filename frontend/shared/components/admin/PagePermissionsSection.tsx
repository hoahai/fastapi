...
export type PagePermissionsSectionProps = {
  appCode: string;
  appLabel: string;
    ...
    <SectionCard
      ...
      actions={(
        <button
          ...
          className="inline-flex h-8 items-center gap-1.5 rounded-md border border-slate-200 bg-white px-2.5 text-xs font-medium text-slate-700 shadow-sm transition-colors hover:bg-slate-50 disabled:pointer-events-none disabled:opacity-60"
        >
          <RefreshCw className={`size-3.5 ${refreshing || backgroundRefreshing ? "animate-spin" : ""}`} />
          Refresh
        </button>
      )}
    >
                  ...
                  <article key={user.userId} aria-busy={saving} className="relative rounded-xl border border-slate-200 bg-white p-3">
                    ...
                    <div className="mt-2 grid gap-1 sm:grid-cols-2">
                      {availablePages.map((page) => {
                        ...
                        return (
                          <TooltipTarget text={page.route} key={page.key}>
                            <label
                              htmlFor={inputId}
                    ...
                    {dirty && canManage ? (
                      <div className="mt-3 flex flex-wrap items-center justify-end gap-2">
                        <button
                          ...
                          className="inline-flex h-8 items-center justify-center gap-1.5 rounded-md border border-slate-200 bg-white px-3 text-xs font-medium text-slate-700 shadow-sm transition-colors hover:bg-slate-50 disabled:pointer-events-none disabled:opacity-60"
                        >
                          Clear restrictions
                        </button>
                        <button
                          ...
                          disabled={saving}
                          className="inline-flex h-8 items-center justify-center gap-1.5 rounded-md border border-blue-200 bg-blue-50 px-3 text-xs font-medium text-blue-700 shadow-sm transition-colors hover:bg-blue-100 disabled:pointer-events-none disabled:opacity-60"
                        >

...
export function CacheStatusChip({
  ...
  return (
    <div className={cn("group relative inline-flex max-w-full pointer-events-auto", containerClassName)}>
      <button
        type="button"
        onClick={onRefresh}
        disabled={disabled}
      ...
      <span
        id={tooltipId}
        role="tooltip"
        className="pointer-events-none absolute bottom-[calc(100%+0.45rem)] left-1/2 z-40 -translate-x-1/2 whitespace-nowrap rounded-md bg-slate-900 px-2 py-1 text-[11px] font-medium text-white opacity-0 shadow-sm transition-opacity group-hover:opacity-100 group-focus-within:opacity-100"
      >
